...
APP_BASE_URL="${1:?Usage: build.sh <APP_BASE_URL>}"

# Load other environment variables from project root .env (APP_BASE_URL is not read from here)
ENV_FILE="${ENV_FILE:-../../.env}"
if [ -f "$ENV_FILE" ]; then
  export $(grep -v '^\s*#' "$ENV_FILE" | sed 's/[[:space:]]*#.*//' | grep -v '^\s*$' | grep -v '^APP_BASE_URL=' | xargs)
fi
...
echo ">>> Building Flutter Web (APP_BASE_URL=$APP_BASE_URL)..."
